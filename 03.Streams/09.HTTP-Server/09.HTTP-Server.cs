...
class Program
{
    static void Main()
    {
        ...
        TcpListener server = new TcpListener(localAddress, 1337);

        server.Start();
        Console.WriteLine($"Waiting for a connection on {localAddress}:{1337} ... ");
        ...
        while (true)
        {
            using (NetworkStream stream = server.AcceptTcpClient().GetStream())
            {
                ...
                if (url.Equals("/"))
                {
                    requestedPage = $@"..\..\HTML-Files\index.html";
                }
                else
                {
                    requestedPage = $@"..\..\HTML-Files\{url.Substring(url.IndexOf('/'))}";
                    ...
                    {
                        requestedPage = $@"..\..\HTML-Files\error.html";
                    }
                    else
                    {
